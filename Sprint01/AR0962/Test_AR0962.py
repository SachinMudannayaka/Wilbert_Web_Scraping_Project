...
from AR0962 import (main,log_error,create_final_json_file,errors,uniqueIdentity,region,jurisdiction,category,title,casKeyValue)
headers = {
    "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Safari/537.36"
}

class Test_AR0962(unittest.TestCase):
    ...
    def test_get_all_links(self):
        ...
        result = AR0962.get_all_links(url)
        self.assertIsInstance(result, list)
        self.assertEqual(result, [url])

    @patch("AR0962.requests.Session.get")
    def test_download_csv_and_parse_with_unsafe_tls(self, mock_get):
        ...
        session = AR0962.requests.Session()
        url = "https://fakeurl.com"
        headers = {}

        df, second_col_name = AR0962.download_csv_and_parse_with_unsafe_tls(session, url, headers)
        self.assertIsInstance(df, pd.DataFrame)
        self.assertEqual(second_col_name, "CAS")
        self.assertEqual(len(df), 2)
        self.assertListEqual(list(df.columns), ["Name", "CAS", "Value"])

    def test_append_csv_data_to_global_data(self):
        df = pd.DataFrame([
            {"Col1": "A", "Col2": 1},
            {"Col1": "B", "Col2": 2}
        ])
        ...
        AR0962.append_csv_data_to_global_data(df)

        self.assertEqual(len(AR0962.data), 1)
        self.assertEqual(AR0962.data[0], {"Col1": "B", "Col2": 2})    
