...
from AR0177 import (main,log_error,create_final_json_file,errors,uniqueIdentity,region,jurisdiction,category,title,casKeyValue,getSoup,get_search_inventory_link,find_download_inventory_link,get_excel_data_as_list)
headers = {
    "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Safari/537.36"
}

class Test_AR0177(unittest.TestCase):
    ...
    @patch('AR0177.requests.get')
    def test_successful_get_and_parse(self, mock_get):
        mock_response = Mock()
    ...
    @patch('AR0177.requests.get')
    @patch('AR0177.log_error')
    def test_http_error(self, mock_log_error, mock_get):
        mock_response = Mock()
        mock_response.raise_for_status.side_effect = requests.exceptions.HTTPError("404 Not Found")
        mock_get.return_value = mock_response

        soup = getSoup("http://example.com/404")
        self.assertIsNone(soup)
        mock_log_error.assert_called_once()

    @patch('AR0177.requests.get', side_effect=requests.exceptions.ConnectionError("No connection"))
    @patch('AR0177.log_error')
    def test_request_exception(self, mock_log_error, mock_get):
        soup = getSoup("http://example.com/fail")
        self.assertIsNone(soup)
        mock_log_error.assert_called_once()

    @patch('AR0177.requests.get')
    @patch('AR0177.log_error')
    def test_parse_exception(self, mock_log_error, mock_get):
        ...
        with patch('AR0177.BeautifulSoup', side_effect=Exception("Parse Error")):
            soup = getSoup("http://example.com")
            self.assertIsNone(soup)
            mock_log_error.assert_called_once() 
    ...
    def test_exception_handling(self, mock_log_error):
        result = get_search_inventory_link(None, self.base_url)
    ...
    def test_valid_download_link(self):
        html = '<input id="downloadInventoryId" value="abc123">'
        soup = BeautifulSoup(html, 'html.parser')
        result = find_download_inventory_link(soup, self.base_url)
        expected = "https://example.com/_entity/annotation/abc123"
        self.assertEqual(result, expected)

    def test_missing_input_tag(self):
        html = '<div>No input here</div>'
        soup = BeautifulSoup(html, 'html.parser')
        result = find_download_inventory_link(soup, self.base_url)
        self.assertIsNone(result)

    def test_input_tag_without_value(self):
        html = '<input id="downloadInventoryId">'
        soup = BeautifulSoup(html, 'html.parser')
        result = find_download_inventory_link(soup, self.base_url)
        self.assertIsNone(result)
    ...
    @patch('AR0177.log_error')
    def test_exception_handling(self, mock_log_error):
        result = find_download_inventory_link(None, self.base_url)
        self.assertIsNone(result)
        mock_log_error.assert_called_once() 


    @patch('AR0177.requests.get')
    def test_valid_excel(self, mock_get):
        ...
        result = get_excel_data_as_list("http://example.com/test.xlsx")
    ...
    def test_http_error(self, mock_get):
        ...
        mock_get.return_value = mock_response
        result = get_excel_data_as_list("http://example.com/404.xlsx")
        self.assertEqual(result, [])

    @patch('AR0177.requests.get', side_effect=Exception("Connection error"))
    def test_request_error(self, mock_get):
        result = get_excel_data_as_list("http://example.com/error.xlsx")
        self.assertEqual(result, [])

    @patch('AR0177.requests.get')
    def test_excel_processing_error(self, mock_get):
        mock_response = Mock()
        mock_response.content = b"not excel"
        mock_response.raise_for_status = Mock()
        mock_get.return_value = mock_response

        result = get_excel_data_as_list("http://example.com/bad.xlsx")
        self.assertEqual(result, [])     
